...
context = zmq.Context()
socket = {}
socket['web-pull'] = context.socket(zmq.PULL)
socket['web-pull'].connect("tcp://127.0.0.1:2881")
...
def acquire_images(image_result, count, isSaved):
    try:
        ...
        if image_result.IsIncomplete():
            ...
            MQ.pub('/img-recv', img_str)
            print(fr'0MQ 전송 완료 FLIR ---> WEB')
...
def open_mq():
    global cam_list, cam, system, status

    subject = Subject()
    def subscribeSocket(result):
        global cam, cam_list, system
        (topic, msg) = result
        ...
        if topic == '':  # 정상
            ...
            print("ReactiveX: 소프트웨어 트리거 온.")
            triggerSet(cam)
        elif topic == '/cam-release':
            subscription.dispose()
            print("open_mq: 카메라 객체 반환")
            try:
                cam.UnregisterEventHandler(eventHandler)
                cam.EndAcquisition()
                if not setNodeValue('TriggerMode', 'Off'):
                    print('open_mq: TriggerMode 설정 실패')
            except Exception as e:
                print('open_mq: cam.EndAcquisition()', e)
                pass
            try:
                cam.DeInit()
            except Exception as e:
                print('open_mq: cam.DeInit()~', e)
                pass
            del cam
            cam_list.Clear()
            system.ReleaseInstance()
            print("open_mq: 잠시후 프로그램이 종료됩니다 1")
            MQ.stop()
            exit(0)
    ...
    MQ.init()
    MQ.keyDeclare()
    MQ.addSubject(subscribeSocket)
    MQ.sub(['/img-stop', '/start', '/cam-release', '/img-start'])

if __name__ == '__main__':
    system, cam_list = main1()
